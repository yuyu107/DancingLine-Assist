...
function Scan-CurrentLevel {try{$state.Text=$auto.Scan()}catch{$state.Text=$_.Exception.GetBaseException().Message}}
Add-Button '1. 识别当前关卡（F6）' 20 130 {Scan-CurrentLevel}
...
Add-Button '2. 启动自动游玩（F7）' 225 130 {Start-AutoPlay}
...
$note.Text="F6 识别当前关卡，F7 启动/恢复自动游玩，F8 停止；位置模式接近引导点时发送空格。`r`n遮罩只显示工具状态，不修改游戏画面；独占全屏下可能不会显示。"
...
$overlayToggle.Checked=$true;$overlayToggle.SetBounds(20,280,590,23);$form.Controls.Add($overlayToggle)
...
$leadLabel=New-Object Windows.Forms.Label;$leadLabel.Text='按键提前量（毫秒；严格路段可试 5）';$leadLabel.SetBounds(20,310,330,25);$form.Controls.Add($leadLabel)
$leadInput=New-Object Windows.Forms.NumericUpDown;$leadInput.Minimum=0;$leadInput.Maximum=40;$leadInput.Value=20;$leadInput.SetBounds(365,307,75,28);$form.Controls.Add($leadInput)
...
$timer.Add_Tick({
 $now=[DateTime]::UtcNow
 if(-not $auto.Running -and $auto.ConsumeScanHotkey()){Scan-CurrentLevel}
