...
$form.Text='跳舞的线 · 引导线与自动游玩 0.4.5 路线对齐等待版'
$form.ClientSize=New-Object Drawing.Size(640,410)
...
$state.Text='等待操作';$state.SetBounds(20,250,600,90);$form.Controls.Add($state)
...
Add-Button '2. 启动自动游玩' 225 130 {try{$auto.Start([int]$offset.Value);$state.Text='已准备；切回游戏后手动开始或继续，F8 停止。'}catch{$state.Text=$_.Exception.GetBaseException().Message}}
...
$note.SetBounds(20,345,600,55);$form.Controls.Add($note)
